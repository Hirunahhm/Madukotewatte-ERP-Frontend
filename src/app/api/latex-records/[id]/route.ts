...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        const data = await backendJson(`/latex-records/${id}`);
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        const data = await backendJson(`/latex-records/${id}`, { method: "PUT", body: JSON.stringify(body) });
        return NextResponse.json(data);
    } catch (err) {
        if (err instanceof BackendError) return NextResponse.json({ message: err.message }, { status: err.status });
        return NextResponse.json({ message: "Unable to reach the server." }, { status: 503 });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    try {
        await backendJson(`/latex-records/${id}`, { method: "DELETE" });
