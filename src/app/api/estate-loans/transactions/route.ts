...
export async function GET(req: NextRequest) {
    try {
        ...
        const data = await backendJson(`/estate-loans/transactions${qs ? `?${qs}` : ""}`);
...
export async function POST(req: NextRequest) {
    try {
        ...
        const data = await backendJson("/estate-loans/transactions", { method: "POST", body: JSON.stringify(body) });
