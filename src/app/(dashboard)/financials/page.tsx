            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
                    <div className="flex items-start justify-between mb-8">
                        <div>
                            <h2 className="text-base font-semibold leading-6 text-gray-900 flex items-center gap-2">
                            ...
                            <p className="text-sm text-gray-500 mt-1 max-w-[200px]">Rubber Grade RSS1 Market Price Trends (USD/kg)</p>
                        </div>
                ...
                <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm flex flex-col">
                    <div className="flex items-start justify-between mb-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1 max-w-[200px]">Monthly Expenditure by Functional Category</p>
                        </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex bg-gray-50 p-1 rounded-lg border border-gray-100">
                            <button className="text-xs font-semibold bg-brand-500 text-white shadow-sm px-4 py-1.5 rounded">All</button>
                            <button className="text-xs font-semibold px-4 py-1.5 text-gray-500 hover:text-gray-900">Income</button>
                            <button className="text-xs font-semibold px-4 py-1.5 text-gray-500 hover:text-gray-900">Expenses</button>
                        </div>
                        <div className="relative w-64 hidden md:block">
                            <Filter className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                            <input type="text" placeholder="Filter transactions..." className="w-full pl-9 pr-4 py-1.5 bg-white border border-gray-200 rounded-md text-sm focus:ring-1 focus:ring-brand-500" />
                        </div>
                    </div>
                </div>
                            ...
                            {transactions.map((tx, idx) => (
                                <tr key={idx} className="hover:bg-gray-50/50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${tx.type === 'income' ? 'bg-gray-100 text-gray-600' : 'bg-red-50 text-red-600'}`}>
                                            {tx.type}
                                        </span>
                                    </td>
                                    <td className={`px-6 py-4 font-bold ${tx.type === 'income' ? 'text-gray-900' : 'text-gray-900'}`}>
                                        {tx.amount}
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${tx.status === 'completed' ? 'bg-gray-100 text-gray-500 border border-gray-200' : 'bg-gray-100 text-gray-500'}`}>
                                            {tx.status}
                                        </span>
                                    </td>
                ...
                <div className="p-4 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
                    ...
                    <div className="flex items-center gap-2">
                        <button className="px-4 py-1.5 rounded font-semibold text-gray-500 hover:text-gray-900">Previous</button>
                        <button className="px-4 py-1.5 rounded font-semibold bg-gray-50 text-gray-900 border border-gray-200 shadow-sm">Next Page</button>
                    </div>
                </div>
            </div>
        </div>
