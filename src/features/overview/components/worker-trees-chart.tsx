...
import { useWorkerTreesRanking } from "@/features/overview/hooks/use-overview";

export function WorkerTreesChart() {
    const { ranking, isLoading } = useWorkerTreesRanking();
    const chartData = ranking.map((r) => ({ name: r.employeeName, trees: r.trees }));
