...
export function VolumeTrendsChart() {
    ...
    // Fetch trends for the latest load
    const { data: trendsData, isLoading: isLoadingTrends } = useLoadTrends(latestLoadId, days);
    
    const isLoading = isLoadingLoads || isLoadingTrends;
    const chartData = Array.isArray(trendsData) ? trendsData : [];

    return (
        <Card className="lg:col-span-2 shadow-sm gap-0 p-6 relative">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <CardDescription>Daily actual collection vs. weekly target forecasts {latestLoadId && <span className="lowercase font-mono text-[9px] bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded ml-2 text-gray-500">load: {latestLoadId.slice(0,8)}</span>}</CardDescription>
                </div>
