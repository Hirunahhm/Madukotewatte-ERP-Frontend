...
export function ManiocLoadTracker() {
    const [dialogOpen, setDialogOpen] = useState(false);
    const [selectedLoad, setSelectedLoad] = useState<LoadData | null>(null);

    const { data, isLoading } = useLoads({ loadType: "manioc", size: 10, sort: "startDate,desc" });
        ...
        <Card className="shadow-sm gap-0 p-6 flex flex-col">
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-base font-semibold">Manioc Batch Tracker</CardTitle>
                </div>
            ...
            <CardDescription className="mb-6">Manioc batch processing status</CardDescription>
            ...
            <div className="space-y-4 flex-1">
                {isLoading ? (
                    ...
                    <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-8">No manioc loads found. Create one to get started.</p>
                ) : (
                    loads.map((load) => (
                        <div
                            key={load.loadId}
