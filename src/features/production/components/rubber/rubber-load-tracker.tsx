...
export function RubberLoadTracker() {
    ...
    const { data, isLoading } = useLoads({ loadType: "scrap", size: 10, sort: "startDate,desc" });
        ...
        <Card className="shadow-sm gap-0 p-6 flex flex-col">
            ...
            <div className="space-y-4 flex-1">
                {isLoading ? (
                    ...
                    <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-8">No scrap loads found. Create one to get started.</p>
                ) : (
                    loads.map((load) => (
                        <div
                            key={load.loadId}
