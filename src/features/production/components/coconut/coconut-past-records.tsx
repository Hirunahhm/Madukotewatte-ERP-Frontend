...
import {
    useCoconutRecords,
    useUpdateCoconutRecord,
    useDeleteCoconutRecord,
} from "@/features/production/hooks/use-coconut-production";
import { COCONUT_VARIETIES } from "@/features/production/types/coconut-production.types";
import type { CoconutRecord } from "@/features/production/types/coconut-production.types";

function EditRecordDialog({ record, onClose }: { record: CoconutRecord | null; onClose: () => void }) {
    const [variety, setVariety] = useState("");
    const [nutCount, setNutCount] = useState("");
    const [massKg, setMassKg] = useState("");
    const [date, setDate] = useState("");
    const updateMutation = useUpdateCoconutRecord();

    useEffect(() => {
        if (record) {
            setVariety(record.variety);
            setNutCount(record.nutCount != null ? String(record.nutCount) : "");
    ...
    return (
        <Dialog open={record !== null} onOpenChange={(open) => { if (!open) onClose(); }}>
            <DialogContent className="sm:max-w-sm">
                <DialogHeader>
                    <DialogTitle>Edit Coconut Record</DialogTitle>
                </DialogHeader>
                <div className="space-y-3 py-2">
                    ...
                    <div className="space-y-1.5">
                        <Label>Nuts</Label>
                        <Input type="number" value={nutCount} onChange={(e) => setNutCount(e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                        <Label>Mass (kg)</Label>
                    ...
                    <div className="flex gap-2 pt-2">
                        ...
                        <Button className="flex-1 bg-blue-500 hover:bg-blue-600 text-white" onClick={handleSave} disabled={updateMutation.isPending}>
...
export function CoconutPastRecords() {
    const [page, setPage] = useState(0);
    const [from, setFrom] = useState("");
    const [to, setTo] = useState("");
    const [editing, setEditing] = useState<CoconutRecord | null>(null);

    const { data, isLoading } = useCoconutRecords({
        page,
        size: 10,
    ...
    const deleteMutation = useDeleteCoconutRecord();

    async function handleDelete(record: CoconutRecord) {
        if (!window.confirm("Delete this coconut record? This cannot be undone.")) return;
    ...
    return (
        <Card className="shadow-sm gap-0 p-6">
            <div className="flex flex-wrap items-end justify-between gap-3 mb-4">
                <CardTitle className="text-base font-semibold">Past Coconut Records</CardTitle>
                        ...
                        {isLoading ? (
                            <TableRow>
                                <TableCell colSpan={7} className="text-center py-8">
                                    <Loader2 className="w-5 h-5 animate-spin inline text-blue-500" />
                                </TableCell>
                            </TableRow>
                        ) : records.length === 0 ? (
                            <TableRow>
                            ...
                            <TableRow key={row.recordId} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                                ...
                                <TableCell className="text-sm">{row.nutCount ?? "—"}</TableCell>
                                <TableCell className="text-sm font-medium text-blue-600 dark:text-blue-400">{row.massKg != null ? `${row.massKg} kg` : "—"}</TableCell>
                                <TableCell className="text-right">
                                    <div className="flex items-center justify-end gap-1">
                                        <button onClick={() => setEditing(row)} className="w-7 h-7 rounded flex items-center justify-center text-gray-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20">
