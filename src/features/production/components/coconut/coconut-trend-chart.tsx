...
import { useCoconutLoadTrends } from "@/features/production/hooks/use-coconut-production";

export function CoconutTrendChart() {
    ...
    const { data: trendsData, isLoading: isLoadingTrends } = useCoconutLoadTrends(latestLoadId, days);
    ...
    return (
        <Card className="shadow-sm gap-0 p-6 relative">
            ...
            <div className="flex items-center justify-between mb-4">
                <div>
                    <CardTitle className="text-base font-semibold">Daily Harvest Volume</CardTitle>
                    <CardDescription>Coconut mass collected per day {latestLoadId && <span className="lowercase font-mono text-[9px] bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded ml-2 text-gray-500">load: {latestLoadId.slice(0, 8)}</span>}</CardDescription>
                </div>
                    ...
                    {chartData.length === 0 && !isLoading ? (
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={chartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                                ...
                                <Tooltip />
                                <Bar dataKey="actual" fill={chartColors.blue} radius={[4, 4, 0, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    )}
                </NoSSR>
